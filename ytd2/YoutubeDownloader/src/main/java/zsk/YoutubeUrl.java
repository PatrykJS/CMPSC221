...
public class YoutubeUrl {
	...
	private void extractUrlParameters(String url, String videoUrl) {
		try {
			...
			JFCMainClient.debugOutput(String.format("video url stored for later use with key: %s - type: %s  quality: %s size: %s   - %s",this.getHtmlTagId(), this.getHtmlType(), this.getQuality(), this.getSize(), this.getUrl() ));
	...
	public String toString() {
		...
		return (this.isDownloading ? "downloading " : "") +(this.percentage>-1 ? "(" +Integer.toString(this.percentage)+"%) " : "") +(this.title.length() > 0 ? this.title : this.url);
	}
